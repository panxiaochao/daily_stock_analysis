...
import {ApiErrorAlert, Badge, Card, EmptyState, Pagination, StatusDot, Tooltip} from '../components/common';
...
const PerformanceCard: React.FC<{ metrics: PerformanceMetrics; title: string }> = ({metrics, title}) => (
    <Card variant="gradient" padding="md" className="animate-fade-in">
        ...
        <MetricRow label="Avg Days to Hit"
    ...
    return (
        <div className="min-h-full flex flex-col rounded-[1.5rem] bg-transparent">
            ...
            <header className="flex-shrink-0 border-b border-white/5 px-3 py-3 sm:px-4">
                <div className="flex max-w-5xl flex-wrap items-center gap-2">
                    ...
                    <div className="flex items-center gap-2 whitespace-nowrap lg:w-40 lg:justify-between">
                        <span className="text-xs text-muted-text">窗口期</span>
                        <input
                            type="number"
                            min={1}
                            max={120}
                    ...
                    <div className="flex items-center gap-2 whitespace-nowrap">
                        <span className="text-xs text-muted-text">起始日期：</span>
                        <input
                            type="date"
                ...
                <p className="mt-2 text-xs text-muted-text">
                    {isNextDayValidation
                        ...
                        : '使用 window = 1 来审查 AI 预测与下一个交易日收盘价的对比情况。'}
                </p>
            </header>
            ...
            <main className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden p-3 lg:flex-row">
                ...
                <div
                    className="flex max-h-[38vh] flex-col gap-3 overflow-y-auto lg:max-h-none lg:w-60 lg:flex-shrink-0">
                    {isLoadingPerf ? (
                        ...
                        <EmptyState
                            title="暂无指标"
                            description="运行回测以生成投资组合层面的绩效指标。"
                        ...
                        <div className="animate-fade-in">
                            <div className="backtest-table-toolbar">
                                <div className="backtest-table-toolbar-meta">
                                    <span
                                        className="label-uppercase">{isNextDayValidation ? 'Next-Day Validation' : 'Result Set'}</span>
                                        ...
                                        >
                                            <td className="backtest-table-cell backtest-table-code">
                                                <div className="flex flex-col">
                                                    <span>{row.code}</span>
                                                    <span
                                                        className="text-xs text-muted-text">{row.stockName || '--'}</span>
                                                </div>
                                            </td>
                                                ...
                                                {(row.trendPrediction || row.operationAdvice) ? (
                                                    ...
                                                    >
                                                        <div className="flex flex-col gap-1">
                                                            <span
                                                                className="block truncate">{row.trendPrediction || '--'}</span>
                                                            <span
                                                                className="block truncate text-xs text-secondary-text">{row.operationAdvice || '--'}</span>
                                                        </div>
                                                    </Tooltip>
                                                ) : (
                                                    '--'
